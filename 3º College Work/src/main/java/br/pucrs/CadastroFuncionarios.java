...
public class CadastroFuncionarios {
    ...
    public boolean saveBinaryFile(String nomeArq) {
        Path path1 = Paths.get(nomeArq);
        try (ObjectOutputStream writer = new ObjectOutputStream(Files.newOutputStream(path1))) {
            for(Funcionario f: lista)
                writer.writeObject(f.toString());
        }
        catch (IOException x) {
            System.err.format("Erro de E/S: %s%n", x);
        }
    ...
    public boolean readBinaryFile(String nomeArq) {
        Path path1 = Paths.get(nomeArq);
        Funcionario funcionario = null;

        try (ObjectInputStream reader = new ObjectInputStream(path1) {
            funcionario = (Funcionario) reader.readObject();
        }
        catch (IOException x) {
            System.err.format("Erro de E/S: %s%n", x);
        }
